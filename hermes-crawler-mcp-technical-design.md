...
- ✅ 容器化验收：只读根文件系统 + `cap_drop ALL` + `no-new-privileges` + 非 root（uid 1000）下浏览器仍可运行，私网/元数据地址阻断实测生效。
- ⚠️ 待办：StealthyFetcher 依赖的 camoufox 在容器内为按需下载，需在镜像构建阶段用 `camoufox fetch` 预置到共享路径（当前 HTTP 与 Dynamic 两层已在容器内实测可用，Stealth 层浏览器二进制预置留作生产化前补齐）。
